"""
Directors Cut v2 — HF Space Backend (Production)
Route: POST /generate-video  → returns video blob directly
Route: GET  /health          → returns status JSON
"""
...
{{
  ...
  "scenes": [
    {{"scene_number":1,"duration":5.0,"motion":"slow_push_in",
      "transition":"fade","intensity":"opening","direction_note":"brief note"}}
  ]
}}
...
Allowed pacing: slow, moderate, dynamic, dramatic
Allowed zoom_intensity: low, medium, high

Rules:
- If audio given: scene durations MUST sum to {dur_info}
- No two consecutive scenes same motion
- min scene duration: 3.0s, max: 18s"""
...
def get_zoom_filter(style: str, duration: float, zoom_intensity: str,
                    quality: str) -> str:
    fps    = 25
...
def merge_clips_xfade(clip_paths: list, hold_times: list, scene_transitions: list,
    ...
    """One-pass xfade merge — 4-5x faster than sequential."""
    n = len(clip_paths)
    if n == 1:
...
def build_video(job_dir: Path, image_paths: list, audio_path,
                quality: str, prompt: str, scene_duration: float,
                output_path: Path):
    n    = len(image_paths)
    prof = QUALITY_PROFILES.get(quality, QUALITY_PROFILES[DEFAULT_QUALITY])
    ...
    print(f"[Build] {plan.get('pacing')} pacing | {plan.get('mood')} | "
          f"{plan.get('zoom_intensity')} zoom | q={quality}")
    ...
    for i, img_path in enumerate(image_paths):
        ...
        print(f"[Build] Clip {i+1}/{n} — {motion}")
    ...
    if audio_path and os.path.exists(audio_path):
        ...
        for attempt_cmd in [
            ...
            if proc.returncode == 0:
                print("[Build] Audio mixed"); break
        else:
            raise RuntimeError("Audio mix failed on all attempts")
    else:
        shutil.copy(str(raw_video), str(output_path))

    print(f"[Build] Output: {output_path} ({output_path.stat().st_size//1024} KB)")
...
def generate_video():
    ...
    try:
        ...
        build_video(job_dir, image_paths, audio_path,
                    quality, prompt, scene_duration, output_path)

        resp = send_file(str(output_path), mimetype="video/mp4",
                         as_attachment=False,
