"""
Directors Cut v2 — HF Space Backend (Production v3)
CORS: manual (no flask_cors conflict)
POST /generate-video  → { job_id }
...
def call_gemini(contents):
    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2: raise RuntimeError(f"Gemini failed: {e}")
            time.sleep(2 ** attempt)


def extract_gemini_text(resp):
    try: return resp["candidates"][0]["content"]["parts"][0]["text"]
    except (KeyError, IndexError) as e: raise RuntimeError(f"Gemini parse error: {e}")


def generate_fallback_plan(n, audio_duration, scene_duration=5.0):
...
def validate_plan(plan, n, audio_duration):
    scenes = plan.get("scenes", [])
    if len(scenes) < n:
        return generate_fallback_plan(n, audio_duration)
    scenes = scenes[:n]
    for i, s in enumerate(scenes):
        if s.get("motion") not in MOTION_STYLES: s["motion"] = MOTION_STYLES[i % len(MOTION_STYLES)]
        if s.get("transition") not in TRANSITION_TYPES: s["transition"] = "fade"
    ...
    plan["scenes"] = scenes
    if plan.get("pacing") not in PACING_VALUES: plan["pacing"] = "moderate"
    if plan.get("zoom_intensity") not in ZOOM_INTENSITIES: plan["zoom_intensity"] = "medium"
    td = float(plan.get("transition_duration", 1.2))
    plan["transition_duration"] = max(0.5, min(2.5, td))
    return plan


def ai_director_plan(scene_names, audio_duration, prompt, scene_duration):
    n = len(scene_names)
    if not GEMINI_API_KEY:
    ...
    txt = f"""Cinematic director plan for {n} scenes.
Scenes:\n{scene_list}\nAudio: {dur_info}, default duration: {scene_duration}s{hint}

Return ONLY JSON:
{{"pacing":"moderate","mood":"cinematic","zoom_intensity":"medium","transition_duration":1.2,
"scenes":[{{"scene_number":1,"duration":5.0,"motion":"slow_push_in","transition":"fade","intensity":"opening","direction_note":"note"}}]}}
...
pacing: slow|moderate|dynamic|dramatic  zoom_intensity: low|medium|high
Rules: no consecutive same motion; min 3s/scene; sum durations={dur_info} if audio given."""
    try:
        resp = call_gemini([{"role":"user","parts":[{"text":txt}]}])
        plan = json.loads(extract_gemini_text(resp))
        return validate_plan(plan, n, audio_duration)
    except Exception as e:
        print(f"[AI Director] Gemini failed: {e}")
        return generate_fallback_plan(n, audio_duration, scene_duration)


# ═══ FFMPEG ═══════════════════════════════════════════════════════════════════

def get_zoom_filter(style, duration, zoom_intensity, quality):
    fps = 25; frames = max(int(duration * fps), 30)
    prof = QUALITY_PROFILES.get(quality, QUALITY_PROFILES[DEFAULT_QUALITY])
    W, H = prof["W"], prof["H"]; SW, SH = W*2, H*2
    cx = "iw/2-(iw/zoom/2)"; cy = "ih/2-(ih/zoom/2)"
    mult = {"low":0.55,"medium":1.0,"high":1.5}.get(zoom_intensity, 1.0)
    slow_d = max(1, int(W*0.22/frames)); fast_d = max(1, int(W*0.30/frames))
    base = f"scale={SW}:{SH}:force_original_aspect_ratio=increase,crop={SW}:{SH},"
    if style == "slow_push_in":
        e=min(1.0+0.38*mult,1.65); r=(e-1.0)/frames
        zp=f"zoompan=z='min(1.0+on*{r:.5f},{e:.3f})':x='{cx}':y='{cy}':d={frames}:s={W}x{H}:fps={fps}"
    elif style == "slow_pull_back":
        s=min(1.0+0.38*mult,1.65); r=(s-1.0)/frames
        zp=(f"zoompan=z='if(eq(on\\,1)\\,{s:.3f}\\,max({s:.3f}-on*{r:.5f}\\,1.0))':"
            f"x='{cx}':y='{cy}':d={frames}:s={W}x{H}:fps={fps}")
    elif style == "drift_left":
        zv=1.0+0.2*mult
        zp=(f"zoompan=z='{zv:.3f}':x='iw/2-(iw/zoom/2)+{slow_d}*on':y='{cy}':d={frames}:s={W}x{H}:fps={fps}")
    elif style == "drift_right":
        zv=1.0+0.2*mult
        zp=(f"zoompan=z='{zv:.3f}':x='iw/2-(iw/zoom/2)-{slow_d}*on':y='{cy}':d={frames}:s={W}x{H}:fps={fps}")
    elif style == "dramatic_push":
        e=min(1.0+0.55*mult,1.80); r=(e-1.0)/frames
        zp=(f"zoompan=z='min(1.0+on*{r:.5f},{e:.3f})':x='iw/2-(iw/zoom/2)':y='ih*0.58-(ih/zoom/2)':d={frames}:s={W}x{H}:fps={fps}")
    elif style == "arc_left":
        e=min(1.1+0.28*mult,1.55); r=(e-1.1)/frames
        zp=(f"zoompan=z='min(1.1+on*{r:.5f},{e:.3f})':x='iw/2-(iw/zoom/2)+{fast_d}*on':y='{cy}':d={frames}:s={W}x{H}:fps={fps}")
    elif style == "arc_right":
        e=min(1.1+0.28*mult,1.55); r=(e-1.1)/frames
        zp=(f"zoompan=z='min(1.1+on*{r:.5f},{e:.3f})':x='iw/2-(iw/zoom/2)-{fast_d}*on':y='{cy}':d={frames}:s={W}x{H}:fps={fps}")
    else:
        e=1.0+0.09*mult; r=(e-1.0)/frames
        zp=(f"zoompan=z='min(1.0+on*{r:.5f},{e:.3f})':x='{cx}':y='{cy}':d={frames}:s={W}x{H}:fps={fps}")
    return base + zp
...
def merge_clips_xfade(clip_paths, hold_times, scene_transitions, transition_duration, output_path, quality):
    n = len(clip_paths)
    if n == 1: shutil.copy(clip_paths[0], output_path); return
    prof = QUALITY_PROFILES.get(quality, QUALITY_PROFILES[DEFAULT_QUALITY])
    inputs = []
    for p in clip_paths: inputs += ["-i", p]
    parts, current, cum = [], "[0:v]", 0.0
    for i in range(1, n):
        cum += hold_times[i-1]
        ...
        parts.append(f"{current}[{i}:v]xfade=transition={xft}:duration={transition_duration:.3f}:offset={cum:.3f}{out}")
        current = out
    ...
    except subprocess.TimeoutExpired: proc.kill(); proc.communicate(); raise RuntimeError("xfade merge timed out")
...
def build_video(job_id, job_dir, image_paths, audio_path, quality, prompt, scene_duration, output_path):
    ...
    try:
        ...
        n = len(image_paths)
        prof = QUALITY_PROFILES.get(quality, QUALITY_PROFILES[DEFAULT_QUALITY])
        audio_duration = get_audio_duration(audio_path) if audio_path and os.path.exists(audio_path) else None
        upd(10,"Generating cinematic plan...")
        plan = ai_director_plan([Path(p).name for p in image_paths], audio_duration, prompt, scene_duration)
        zoom_intensity = plan.get("zoom_intensity","medium")
        transition_duration = float(plan.get("transition_duration",1.2))
        scenes = plan.get("scenes",[])
        upd(15,f"Plan: {plan.get('pacing')} · {plan.get('mood')}")

        scene_durations = [max(3.0, float(scenes[i].get("duration",scene_duration)) if i < len(scenes) else scene_duration) for i in range(n)]
        if audio_duration and audio_duration > 0:
        ...
        if audio_duration and audio_duration > 0:
            ...
            if expected < audio_duration + 0.8:
                ...
                hold_times[-1] += deficit; clip_durations[-1] = hold_times[-1]+transition_duration

        clip_paths = []
        for i, img_path in enumerate(image_paths):
            motion = (scenes[i].get("motion",MOTION_STYLES[i%len(MOTION_STYLES)]) if i < len(scenes) else MOTION_STYLES[i%len(MOTION_STYLES)])
            if motion not in MOTION_STYLES: motion = MOTION_STYLES[i%len(MOTION_STYLES)]
            ...
            if proc.returncode != 0: raise RuntimeError(f"Clip {i+1} failed:\n{se[-800:].decode('utf-8','replace')}")
            clip_paths.append(str(clip_out))
            upd(15+int((i+1)/n*60), f"Scene {i+1}/{n} — {motion.replace('_',' ')}")

        upd(77,"Weaving transitions...")
        raw_video = job_dir / "raw_video.mp4"
        merge_clips_xfade(clip_paths, hold_times, [s.get("transition","fade") for s in scenes],
                          transition_duration, str(raw_video), quality)
        upd(90,"Syncing audio...")
        if audio_path and os.path.exists(audio_path):
            fade_st = max(0.0, (audio_duration or 0)-1.0)
            cmds = [
                ["ffmpeg","-y","-i",str(raw_video),"-i",audio_path,"-c:v","copy","-c:a","aac",
                 "-b:a","192k","-af",f"afade=t=out:st={fade_st:.3f}:d=1.0",
                 "-shortest","-map","0:v:0","-map","1:a:0",str(output_path)],
                ["ffmpeg","-y","-i",str(raw_video),"-i",audio_path,"-c:v","copy","-c:a","aac",
                 "-b:a","192k","-shortest","-map","0:v:0","-map","1:a:0",str(output_path)],
            ]
            for mc in cmds:
                proc = subprocess.Popen(mc, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
                try: _, se = proc.communicate(timeout=300)
                except subprocess.TimeoutExpired: proc.kill(); proc.communicate(); continue
                if proc.returncode == 0: break
            else: raise RuntimeError("Audio mix failed")
        else:
            shutil.copy(str(raw_video), str(output_path))
        kb = output_path.stat().st_size // 1024
        jobs[job_id].update({"status":"done","progress":100,"message":f"Done — {kb} KB","output_path":str(output_path)})
        print(f"[{job_id[:8]}] ✅ {kb} KB")
    except Exception as e:
...
def health():
    return jsonify({"status":"ok","gemini":bool(GEMINI_API_KEY),
                    "quality":DEFAULT_QUALITY,"jobs":len(jobs)})
...
def generate_video():
    try:
        ...
        for i, b64_str in enumerate(images_b64):
            ...
            ext = ".png" if "image/png" in b64_str[:30] else ".webp" if "image/webp" in b64_str[:30] else ".jpg"
            p = job_dir / f"scene_{i:03d}{ext}"
            p.write_bytes(base64.b64decode(raw))
            image_paths.append(str(p))
        image_paths.sort(key=lambda p: int(m.group(1)) if (m:=re.search(r"(\d+)", Path(p).stem)) else 9999)
        ...
        if audio_b64:
            ...
            for mime, ext in [("audio/wav",".wav"),("audio/ogg",".ogg"),
                ...
                if mime in audio_b64[:40]: audio_ext = ext; break
...
def job_status(job_id):
    ...
    j = jobs[job_id]
    return jsonify({"status":j["status"],"progress":j["progress"],"message":j["message"]})
...
def download_video(job_id):
    ...
    def _cleanup():
        time.sleep(300)
        shutil.rmtree(str(Path(out).parent), ignore_errors=True)
